...
- [ ] ⛔ **ALGIZ NELZE OPRAVIT OBRAZEM — `Algiz` a `Thurisaz` sdílejí klíč `protection`** (nález 2026-09-09; šest kandidátů ve dvou kolech, žádný neprošel, všechny bere Thurisaz 4/4). Žádný obraz neodliší dvě runy, které mají týž klíč. **Dvojic se sdíleným klíčem je v `RUNES.k` PĚT** a předpovídají záměny z identitního testu:
  | dvojice | sdílený klíč |
  |---|---|
  | Thurisaz × Algiz | `protection` |
  | Uruz × Hagalaz · Uruz × Dagaz · Hagalaz × Dagaz | `transformation` |
  | Raidho × Ehwaz | `movement` |
  ⚠️ **Je to 📜 kánon (`v2/runar-runes.js`, `RUNES.k`), ne ladění — rozhoduje owner/Cowork, CODE to nemění (§23).** Otázka k rozhodnutí: má se překryv rozlišit (např. Algiz = *warding/being watched over*, Thurisaz = *barrier/threshold*), nebo se má připustit, že si tyhle dvojice zůstanou blízké?
  ⭐ **Do té doby je Algiz neopravitelný** a nemá smysl na něj psát další obrazy.
