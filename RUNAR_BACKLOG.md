...
- [x] **~~ČOČKA ŽIVOTNÍ RUNY — owner zvažuje pozastavení~~ → VYŘEŠENO 2026-09-10 volbou ano/ne** (`user_profiles.life_rune_in_readings`, default zapnuto; přepínač v bočním panelu). ⚠️ **Čeká na SQL od ownera** — `sql/2026-09-10_life_rune_in_readings.sql` + `sql/2026-07-16_user_profiles_column_grants.sql`. Rozhodnutí → `RUNAR_DECISIONS.md` 2026-09-10. Alternativa „životní runa jako doplňující otázka v Ask" **HOTOVA 2026-09-10** — `buildAskPrompt` ji dostává (`_askLifeContext`) a nápověda ji nabízí s dosazenými jmény. Zůstává jen důsledek, který nezmizel: **Ask je premium-only**, takže tahle cesta k životní rune je prémiová. Původní zadání: (KUKY 2026-09-10: *„čočka zabírá místo pro čtení… nejsem si jistý, jestli se hodí dávat ji automaticky"*). **Změřeno: bere si závěrečnou větu v 10/12 čtení** (bez ní 0/12); čtyři různé tažené runy, všech dvanáct závěrů o životní runě. Stojí +5 % délky (EL). Je to **třetí uchazeč o poslední větu** vedle losu tvarů konce. → `RUNAR_EVAL_LOG.md` 2026-09-10.
  **Tři cesty, co owner zvažuje, a co která stojí:**
  1. **Pozastavit** (nejlevnější, vratné) — `_lensContext` vypnout · ⚠️ §13: odstranit i větev v `_priorityContext`, která na čočku odkazuje („let the life-rune lens recede"), jinak prompt mluví o něčem, co tam není · ⚠️ smoke ⑧ `verify_contract_wiring` má `lens` mezi **POVINNÝMI** pro single i všechny spready — musí se upravit spolu s tím · životní runa zůstává vidět ve stromě a v badge, takže se uživateli neztratí.
  2. **Volba jako AREA/seeking/intention** — ⚠️ **není to táž věc:** area/seeking/intention jsou vstupy o OTÁZCE, životní runa je vlastnost ČLOVĚKA (počítá se z data narození). Patří spíš do nastavení („používat mou životní runu ve čteních") než mezi pilulky u čtení. §13 full-path: `startReading` · `resetReader` · 4 spready · shrine parts.
  3. **Do Ask** — ✅ **zvoleno a hotovo 2026-09-10** (rozhodnutí → `RUNAR_DECISIONS.md`). Plumbing `buildAskPrompt(..., life)` existuje; zůstává jen to, že Ask je premium-only.
  **Doporučení CODE:** nejdřív **1** (vratné, měřitelné), o 2/3 rozhodnout až podle toho, jestli čtení bez čočky owner uzná za lepší.
...
- [ ] **ALGIZ: mechanismus NALEZEN čtením textů (Cowork 2026-09-10) — text jde za obrazem, ne za aspektem, a každý zvolený předmět seděl na cizím pozemku** (kámen = Isa/Othila · práh = Thurisaz · ovce+jehně = Berkana · čekání na počasí = Isa). Recept, jak psát Algiz, existuje a je testovatelný bránou → `docs/eval/2026-09-10-algiz-kandidati-texty.md`, sekce VERDIKT. **Není do koše — čeká na priority run od ownera.** Původní stav (2026-09-09): šest kandidátů ve dvou kolech neprošlo.
  ...
  **Stav po 2026-09-10:** příčina popsaná (recept výš) — „napřed zjistit z textů" je HOTOVO, udělal Cowork. Další krok = kandidáti podle receptu přes bránu, až přijdou priority.
- [ ] **Jednoslovný slovník (Cowork 2026-09-09) — mechanicky bezvadný, identitu nekupuje.** 25 run · 119 klíčů · nula kolizí · pět sdílených klíčů rozvázáno. Ale změřeno na týchž čteních (mění se jen legenda soudce): **78 % → 78 %**, kolizní runy **55 % → 55 %**. → **Nenasazovat s očekáváním, že spraví identitu.** ⭐ Jeho skutečná hodnota může být **výš v procesu — jako cíl pro autora** při psaní nových obrazů (přesné slovo místo mlhavé fráze); to je ale **NEZMĚŘENO** a nemá se to tvrdit. Chybí k němu: **IS ekvivalenty** (CODE, jedno kolo s BÍN) ✅ **VERDIKT PŘIJAT (owner 2026-09-10, převzetím doporučení GPT): autorská pomůcka, přesně v zapsaném statusu.** Sedm „kolizních míst" = **watch-set pro tvorbu textu**, ne technické kolize (problém vzniká, až když text Dagazu ZNÍ jako Kenaz — vlastnost obrazu, ne slovníku). Průchod proti DECISIONS/DESIGN udělal Cowork: **čisté**; slovo-po-slovu průchod odložen dle priorit. → `RUNAR_DECISIONS.md` 2026-09-10.
- [ ] ⭐ **Kritérium pro nové obrazy, vypadlo z devíti zabitých kandidátů** (2026-09-09): **situační obraz drží identitu líp než fyzická scéna.** Každý propadák narazil na sloveso nebo předmět souseda — zemětřesení → Hagalaz · „řekneš to u stolu" → Ansuz (voice) · lodě v přístavu → Isa (waiting) · neoraný kámen → Othila (heritage) · ovce s jehnětem → Berkana. Co prošlo 3/3, byl v obou případech **čin s cenou, bez rekvizity patřící jiné runě**. → **„Obraz" nemusí být fyzická scéna** (owner/GPT 2026-09-09: *„nemůže být obraz i slovní vyjádření?"* — odpověď je ANO a je změřená). Zapsat do zadání palety.
- [ ] ⭐ **VÝKLAD JE METAFORA VEDLE METAFORY — runa se v obraze nevysvětlí** (owner 2026-09-09 o větě *„Eihwaz is the tree that sheds the branch it cannot feed and keeps standing"*: *„tohle je prostě metafora vedle metafory. Obraz má tvořit hezký obraz runy, ale pak to musí být smysluplně vysvětleno — co je ta runa v obraze a její význam."*). **Je to VADA, ne vkus:** obraz udělá scénu a esenční řádek k ní přilepí druhou scénu, takže nikde nepadne, co runa je. Změřeno: stopu prostého významu runy nese **17 %** čtení (statická květnová 93 %). ⚠️ **Instrukcí to nejde** — vyzkoušeno a mrtvé je PĚT ploch (zákaz v esenčním řádku · aspektové slovo · celý klíčový seznam · čtyři ukázkové věty v systémovém promptu · `YOUR STANCE: draw the picture and stop there`) → `RUNAR_EVAL_LOG.md` 2026-09-09 (2) a (3). ⚠️ **Není to totéž co krytí 43 % níž:** krytí říká, KTERÝ význam je vůbec dosažitelný — tohle říká, že se ani ten dosažitelný nevysloví.
- [ ] ⭐ **MOŽNOST: KRÁTKÉ OBRAZY (2–4 slova) v `RUNE_IMAGES` místo celých vět** (KUKY 2026-09-14 po testech,
  NErozhodnuto — *„teď jen testujeme, jak co změnit, abychom dosáhli cíle"*). Owner: *„kratší obrazy jsou lepší než
  dlouhé, které Rúnara tlačí do toho, aby v jedné krátké větě napsal všechno z obrazu"*. Doloženo produkcí: dlouhý
  obraz čtení opakuje skoro doslova (Ansuz telefon, Algiz lampa) — viz položka „Obraz se neukazuje, on se OPISUJE" výš.
  **Podmínky z testů:** (a) obraz musí být **stvořený pro tu runu**, aby v něm Rúnar našel, co potřebuje —
  *„the road snowed shut"* sedí Raidhu, ne Ise; *„Raidho potřebuje např. jezdce na koni"* · (b) **aspekt se do čtení
  vtlačí** (`focus on: the road` → „taking its own road"); owner: *„aby Rúnar nehledal pořád, jak dostat ROAD do
  čtení"* — aspekt smí být jen klíč z `RUNES[].k` (smoke ㉤), Raidho `k` = *the road, movement, natural rhythm,
...
- [x] **HOTOVO 2026-09-24 (v4.54, DECISIONS 2026-09-24 (18)) — jméno do promptu přes `rnPrompt()`, rozhraní s glosou dál.** **GLOSA V ISLANDSKÉ HLAVIČCE RUNY se propisuje do čtení** (nález CODE-read 2026-09-22, srovnání modelů).
...
- [ ] ⭐ **KONCE ČTENÍ NIC NEŘÍKAJÍ — pool konců přepracovat** (KUKY 2026-09-16 nad *„You stand in the doorway, and the
  ...
  `_endingShape` z `ENDING_OPEN` / `ENDING_HEAVY` (`v2/runar-utils.js`); do čtení jde jen pokyn tvaru („quiet line that rests",
  „plain choice"…), nic o tom, co má konec člověku dát. Testy → `RUNAR_EVAL_LOG.md` 2026-09-15/16.
  ⚠️ **Nejde jen o pool konců** (ověřeno v promptu 2026-09-18): proti „konec má něco znamenat pro uživatele" dnes stojí
  ČTYŘI pokyny — `YOUR STANCE` *„Draw the picture and stop there — never hand the seeker a conclusion"* · `TWO THINGS THAT
...
- [ ] **MOŽNOST: STŘEDOVÁ VĚTA — jedna věta na začátku systémového promptu, kdo Rúnar je a co dělá** (KUKY 2026-09-18:
  *„zapiš středovou jako potenciální možnost"*; NErozhodnuto). Kandidát Cowork-tune: *„Rúnar není ten, kdo ví, kde stojíš —
  ...
  čtení a Ask pro člověka něco znamenaly — nejdřív rozhodnout položku „KONCE ČTENÍ NIC NEŘÍKAJÍ" (smí konec/Ask říct, co to
  MŮŽE znamenat pro člověka, jako možnost bez diagnózy?); (b) systémový prompt má na čtení slabý měřitelný vliv (výměna čtyř
  vzorů hlasu: 0 rozdílů, `RUNAR_EVAL_LOG.md` 2026-09-15) — abstraktní věta tam nejspíš nezmění nic, test ostrý × vágní ji
  musí ověřit. Owner: *„nedokážu si to představit, je to hodně abstraktní pojem."*
- [ ] **Rozhodnout o losu na esenčním řádku** (změřeno 2026-09-08/09, nenasazeno). Dnešní esenční řádek má **jeden tvar ve 100 %** čtení. Owner četl tři rámy od Coworku a 2026-09-09 je ohodnotil: *„je to jiná větná skladba a to, jak je popsán význam runy, se mění. Je to pozitivní."* Los ze tří rámů, v nichž není dnešní spona, dá rozložení tvarů 33/42/25 a **délku beze změny** (nula navíc na ElevenLabs). Chybí: znění rámů Rúnarovým hlasem + IS + rozhodnutí nasadit. ⚠️ Jeden rám sám = výměna jedné šablony za druhou (ověřeno: rám [A] srazil sponu na 0 %, ale nastoupilo „\<Runa\> sloveso" ve 12/12).
- [ ] **Obraz se neukazuje, on se OPISUJE** (změřeno 2026-09-09). Čtení převezme **50 % slov obrazu**, **místo ve 100 %** a hlavní věc v 92 % (Eihwaz: „farmhouse" 3/3, „rowan" 3/3). Owner: *„obraz má jen ukazovat, ale nemá být stále opakován ve stejném znění."* Návrh k prověření: **předloha + losovaná rodina předmětů** — dá 0 % opakování místa i věci a 3× pestřejší obrazy. ⛔ **ZASTAVENO 2026-09-09 identitním testem na všech 25 runách:** identifikace runy padá z **78 % na 52 %** (náhoda 4 %). Řeší opisování, ale platí za to identitou — a to je horší vada. Pilot na 4 runách ukazoval opak (100 %), byl ale vybraný, ne ověřený (§27). **Nenasazovat, dokud identita nedrží.** → `RUNAR_EVAL_LOG.md` 2026-09-09 (6).
- [ ] **Losovaná rodina předmětů se musí losovat PER RUNA, ne per čtení** (nález 2026-09-09 z konstrukční chyby vlastního testu). Když se losuje per čtení, dostanou všechny runy ve spreadu touž rodinu — přesně ta kolize, které se owner bál. Pojistka **už v kódu je** (motiv-guard, smoke ㉟); stačí ji rozšířit z motivů na rodiny — ⚠️ ale nefunguje, dokud obrazy nemají motivy (položka výš).
- [ ] **Změřit hlídače u životní runy** (`_lensContext`, nález 2026-09-08). Prompt vypíše jméno životní runy a hned zakáže ji jmenovat (*„Never name the life rune"*) — týž vzor jako odstraněný hlídač u `seeking`, ale tady je **jméno potřeba**, takže se nálepka smazat nedá. Zbývá druhá půlka pravidla → `memory/prompt-nepojmenuj-co-hned-zakazes.md`: **ověřit, jestli ten hlídač vůbec něco drží** (u seeking nedržel nic — 0/20 v obou verzích).
- [ ] **Skladba věty jako páka — změřeno, nenasazeno** (2026-09-08, owner: *„ne do produkce, zatím jen tady"*). Horní mez „ne víc než patnáct slov" posadí první větu na 13,0 slova (dnes 17,5); *„jedna myšlenka na větu, žádné vsuvky"* čárky nesrazí (1,00 v obou verzích), ale **zlomí tuhost** (v1 dala přesně 3 čárky ve všech čtyřech čteních, v2 už 3-3-4-3). Délka čtení se nemění → **nula navíc na ElevenLabs**.

- [~] **ČTENÍ JSOU NEÚPLNÁ: obrazy odemykaly jen 43 % významu run — DÁVKA 1 NASAZENA 2026-09-10** (22 obrazů prošlo branou z 36; krytí **43 % → 56 %**, run s jedinou dosažitelnou stránkou **5 → 1**). Hotovo: Ansuz (všech 5 stránek) · Nauthiz · Jera · Mannaz. **Zbývá Eihwaz** — všech 6 kandidátů propadlo, jeho jádro („starý strom, smrt a znovuzrození") je Othilino pole; potřebuje obrazy, kde je ROZDÍL proti dědictví. Původní zadání: (owner 2026-09-09: *„potřebuju dostat ostatní meaning run do obrazu… pokud je Eihwaz yew tree/resilience/endurance/death and rebirth, tak nemůže zůstat jen resilience! Platí pro všechny runy."*). Změřeno napříč všemi **25** runami (⚠️ oprava 2026-09-09: dřív tu stálo 24 — runu **Blank** jsem tiše vyřadil, přitom má vlastní klíče i 3 obrazy):
  | | |
  |---|---|
  ...
  ⭐ **Rozsah práce je spočítaný: chybí 68 obrazů** (dnes 85 → úplné pokrytí 153). Seznam se
...
- [ ] **MOŽNOST C — registr čtený z follow-up otázky místo z nabídky** (Cowork 2026-09-08, owner: *„necháme zatím jako možnost, pokud to ve čteních nebude vidět; ale bylo by dobré to alespoň otestovat, ať víme"*).
  **Návrh:** `seeking` se nevybírá dopředu z pěti nálepek — Ask si registr přečte z toho, JAK je follow-up otázka položená („je tohle správně?" už samo zní jako Confirmation).
  ⭐ **Nejsilnější argument (owner 2026-09-08, vedlejší produkt inventury):** plocha Ask má **nejčistší výstup v celém systému — 4 % nároků na vnitřní stav** (komentář u `r-question` ve `v2/runar-reader.html`). Signál by tedy padal přesně tam, kde se model nejmíň uchyluje k tvrzení o tazateli.
  **Co proti (CODE-tune, ověřeno v kódu):** Ask je **premium-only** (`mode === "ask" && userTier !== "premium"` v claude-proxy) → registr by se stal prémiovou funkcí · `buildAskPrompt(reading, question, runes, lang, corrections, life)` k `seeking` nedosáhne, je to plumbing, ne přeformulování — ⚠️ **ale od 2026-09-10 je pro to precedens**: týmž způsobem tam doputovala životní runa (parametr na konci, staří volající beze změny), takže „nejde to" už neplatí, jen to nikdo neudělal · **není to buď-anebo** (Ask může číst registr z otázky a ve čtení může zůstat dvanáctislovný registr).
  **Trigger:** ⚠️ **Aktualizováno 2026-09-10** — Ask už rozbalovací nápovědu nad polem MÁ (`_askHints()`, staví se ze čtení; pole `ask_placeholders` zaniklo). Tím ale ta otázka nezmizela, jen se přiostřila: nápověda uživatele **navádí**, takže registr, který by se z follow-up otázek četl, si zčásti vyrábíme sami. Kdo bude C stavět, musí napřed rozhodnout, jestli má uživatele navádět (a tím registr *vyrábět*), nebo ne (a číst, co přijde samo). **Bez korpusu skutečných follow-up otázek se to změřit nedá** — to je první krok, a je to práce pro Cowork.
- [x] **§22 — rozpor v čísle „nároky na vnitřní stav" — VYŘEŠENO 2026-09-08.** `runar-reader.html` říkalo 23 %, rejstřík pák 18 %. Dohledáno v archivu evalu: obě čísla jsou z 16. 8., ale **23 % je mezivýsledek, který týž den zneplatnila oprava detektoru** („dřívější hodnoty z dneška neplatí", 15 → 23 → **18**; mimo jiné `finnur` = „najde" se počítalo jako nárok na nitro). Platí **18 %**. Komentář v HTML číslo už neopisuje — nese důvod a odkazuje na vlastníka (§20).

- [ ] **Tři páky pestrosti, které v kódu jsou a do čtení nedojdou** (nález 2026-09-08, inventura + vlastní ověření; měření → `RUNAR_EVAL_LOG.md` 2026-09-08). Vrátit se k tomu, až se bude ladit hlas před spuštěním — **každá z nich je pestrost zadarmo, bez psaní nových obrazů**:
  1. **`SEASON_POOLS` = 133 obrazů, ve čtení 0×.** Ověřeno protlačením: 0 ze 144 dvojic runa × bucket je bez runového kandidáta. Rozhodnout **odstranit, nebo zapojit** — dnes se jen udržují dvojjazyčně a nikam nevedou. Padá s tím i los klíčových slov (`pickedKws`), který visí na téže podmínce. ⚠️ `CLAUDE.md` tvrdila opak; opravena 2026-09-08.
  2. **Sampling NENÍ páka — nejde nastavit.** ⚠️ Ráno 2026-09-08 tu stálo „rozhodnout, jestli to chceme řídit"; to bylo špatně a opravil to pokus o měření: `temperature`, `top_p` i `top_k` vrací na `opus-4-8` **HTTP 400 „deprecated for this model"**, projde jedině `temperature: 1` (= default). Zůstává z toho jen důsledek: bez seedu **nejde golden-verify na text**, jen na prompt — a rozdíl dvou čtení z téhož promptu je dál největší jediný zdroj pestrosti, jen se nedá řídit. Vrátit se k tomu, až se bude měnit model (nový model může parametry zase přijímat).
  3. **Spready nelosují nic** — `_randomAngle`, `_lengthBudget` i `_endingShape` mají každý jediné volání, a to v single builderu. **Devítirunový Yggdrasil má míň proměnných než jednorunové single.** U kříže a Yggdrasilu je důvod v komentáři (v4.13), u noren a podkovy je týž stav **bez zdůvodnění** (§28).
  4. **Sowilo má jediného kandidáta v `deepwinter` i `darkening`** → od září do února dostává pořád týž obraz (*„the year’s first ray of sun touches the mountain peak"*). **Proč:** Sowilo = slunce, takže 3 ze 4 jeho obrazů jsou zatříděné `bright` (jaro–podzim) a zimu pokrývá jediný `cold`. Není to chyba kódu, je to důsledek toho, že se sluneční obrazy přirozeně psaly letní. **Jediná runa z 25, které se to děje** (0 dvojic bez kandidáta, 2 dvojice s jediným — obě Sowilo). Nejlevnější jednotlivá oprava ze seznamu: dopsat Sowilu 1–2 zimní obrazy. ⚠️ Obsah = doména ownera/Coworku, ne CODE — islandská zimní scéna se slunečním motivem se **navrhne a ověří**, nevymýšlí (§23).
- [ ] **Rozpory doc ↔ kód nahlášené inventurou 2026-09-08 — NEOVĚŘENO mnou, ověřit před opravou.** Ať se to neztratí (§22), ale ať to nikdo nebere jako fakt (§23): `runar-character.js:1303` komentář proti kódu `:1307` (EN vazba klíčů) — ⚠️ na tomhle stojí řádek „EN se neváže" v rejstříku pák · zastaralé počty obrazů v komentářích (`:450` „67", `:630` „81" proti 83) · `runar-config.js:415` slibuje shrine dropdown přes `shrine_voice_profile`, který prý nikde jinde neexistuje · `runar-reader.html:258` „Only your name is required" proti `startReading()` · strop Ask 320 natvrdo v `runar-reading.js:571` mimo config (§8) · `buildSysPromptV2` + `DEF_CHAR_V2_EN` jako druhá rozešlá kopie postavy (volají jen `scripts/utils/compare_*.js`) · `VARIABILITY POOLS` v `runar-utils.js` bez volajícího · `supabase/functions/tree-update` bez volajícího a bez komentáře „proč odešel".
...
- [x] **Mytologické obrazy (Óðinn/Nornir)** — ZAVŘENO 2026-08-22 (KUKY: „mytologické obrazy NE!"). Nevrací se; případný budoucí návrat jen přes §26 (očištěný, se slepým testem).
...
- [x] **Ověřit na produkčních datech: `aol`/`intention` v `readings`** — ZMĚŘENO 2026-08-22 (Code na DB dosáhne přes `supabase db query --linked`, „musí owner" už neplatí): 324 čtení · 63 aol (19 %) · 35 intention (11 %). Pro tree lane: větve ze starých čtení budou řídké. Obojí je v UI **volitelné** (pilulky „AREA OF LIFE" a „THIS READING IS FOR" jsou označené OPTIONAL), takže když je testeři přeskakují, obě dokumentované páky §3 (výška z intention, strana z area) budou v produkci **slabé z nedostatku dat**, ne z chyby v kódu.
...
- [~] **Podlaha výšky výstupu 0,50 sráží větve na jednu úroveň** (viz `RUNAR_TREE.md` §5). Při 9 větvích už dvě sedí na 50 %; při `maxMains` 25 by jich tam bylo osmnáct. **V LABU VYŘEŠENO 2026-08-18** (posuvník `exitFloor`, slepených větví 5 → 0; čísla v `RUNAR_TREE.md`). **Produkce ne:** `build_tree_production.py` i `runar-tree-prod.js` mají dál tvrdé `Math.max(0.50, …)` (ověřeno 2026-09-23). Port až poté, co owner v labu dohraje hodnoty — dřív by se přenášel mezistav.
...
- [ ] **`landvættir→Algiz` a `hvalreki→Nauthiz` — až se budou implementovat, platí pro ně zvláštní podmínka (2026-08-19).** V kódu zatím nejsou. Podmínku i důvod vlastní `RUNAR_EVAL_LOG.md` 2026-08-19 — **tady se neopisuje**. Jen připomínka, že to existuje.
- [ ] **Zvěčnit v lanes, že CODE-read smí psát do `RUNAR_DESIGN.md`? (2026-08-18, owner).** Lanes dávají design a docs **Coworku**; CODE-read tam píše (`1eaced0`, `eaa96c0`). **CODE-read k tomu uvádí, že psal na výslovný pokyn ownera („zapiš"), na který se předem ptal** — tedy vědomá výjimka, ne svévole. ⚠️ Je to jeho tvrzení o ownerově pokynu, CODE-tune ho neověřil. Zbývá tedy jediné: **napsat tu výjimku do lanes, nebo ne.** Rozhoduje owner — je to o vlastnictví obsahu.
- [ ] **Dvě otázky na Cowork k `memory/cowork-handoff-quality-bar.md` (2026-08-18, obsah = jejich lane).** (a) **Bod 12** („shrnutí + změněno: nic") říká totéž co `CLAUDE.md:435` *„Handoff má sekci `ZMĚNĚNO:`, i prázdnou"* — jedna informace na dvou místech (§20); má zůstat v obou, nebo odkázat? (b) **Bod 2** předepisuje ≥20–25 vzorků na tvrzení o vzorci a přiznat malé `n` nahlas; ten standard sám stojí na **dvou** handoffech. Doplnit tu větu o sobě? CODE do obsahu nesahá.
- [x] **HOTOVO 2026-08-18: sekce je teď ukazatel na `RUNAR_TREE.md`, pitfall pro `[tune]` zůstal.** Odškrtnuto až 2026-09-23 — oprava proběhla, zápis sem se zapomněl. Původně: **Sekce „Tree of Life — stav" v `CLAUDE.md` nese STAV, který §20.4 zakazuje (2026-08-18, pro CODE-tree).** Čtyři řádky s ✅/❌ („Vizuální engine JE v produkci, 2026-07-10, admin-only beta", „`tree_state`/`tree_readings` DB ❌") — přitom `RUNAR_TREE.md:222` už vlastní *„tabulky zatím NEEXISTUJÍ — je to cíl, ne stav"* a blocker `detectPatterns()` je v tomhle backlogu na :186. CLAUDE.md sám přitom předepisuje „Tree sekce = krátký ukazatel, detail v RUNAR_TREE.md". **CODE-tune to nepřepisuje** (lane), předává se tree lane. ⚠️ Řádka o krmení stromu **regexem přes text čtení** je naopak pitfall pro CODE-tune a musí zůstat — /doctor logika říká pitfalls nechat.
- [x] **UZAVŘENO 2026-08-18 — neduplikuje.** Doslovný překryv hook ↔ CLAUDE.md je 0,3 % proti šumové podlaze 0,2 %; jediný skutečný překryv (3,1 %) je s `working-style.md`, který se při startu nenačítá. Vypadl z toho ale horší nález: repo `CLAUDE.md` je v PODadresáři cwd, takže se při startu nenačítal a po compactu nevkládal — řešeno jednořádkovým importem v `C:\Users\zkuku\CLAUDE.md`. Detail → `RUNAR_DECISIONS.md` 2026-08-18. Původní otázka: nese hook něco, co CLAUDE.md vloží samo? <!-- doc-links:ok 2026-08-18 hook zije MIMO repo a prave to je obsah teto polozky; odkaz musi zustat cteny --> Oficiální docs: *„Project-root CLAUDE.md survives compaction — after /compact, Claude re-reads it from disk and re-injects it."* Náš `~/.claude/runar-context.py` vlévá ~11 kB včetně pravidel. Jestli se to překrývá, je to jedna informace dvěma kanály (§20) a zbytečné tokeny v každé session. ZMĚŘIT (porovnat obsah hooku s CLAUDE.md), pak teprve řezat — netvrdím, že se překrývají.
- [x] **Čočka životní runy nedochází na velké spready** — NÁLEZ BYL ARTEFAKT FIXTURE (ověřeno 2026-08-23 po rozhodnutí ownera „dáme ji tam"): `_lensContext` je ve všech 4 spread builderech a funguje; fixture ㉚ měla lifeRune=R[3] MEZI taženými (slice 0-9) → čočka správně ustoupila → špión hlásil „nedochází" (§19.3 — kontrola na vlastním fixture). Fixture opravena (R[10]), výjimky zúženy. Owner testuje čočku na reálných čteních. Původně: (2026-08-18, nalezla smoke ㉚). `_lensContext` přispívá u `single` a `norns`, ale u `kriz`, `horseshoe` ani `yggdrasil` ne — tam se tedy životní runa jako čočka neuplatní. Nikde není napsané, že to tak má být. Rozhodnout: záměr (velký spread má dost vlastní struktury), nebo mezera jako u ÁVARP? Do rozhodnutí vedeno jako výjimka v `scripts/verify_prompt_levers.js`.
- [x] **UZAVŘENO 2026-08-18 — nepřidává se.** Obrácená páka na 300 čteních (6 ramen, týž den, týž model): vypnutí úhlu u single stejnost NEZVÝŠILO, v islandštině dokonce klesla, a všechny rozdíly jsou menší než rozptyl uvnitř téhož ramene. Čísla → `RUNAR_EVAL_LOG.md` 2026-08-18. Původní stav: **úhel čtení má JEN `single`, ostatních šest cest ne (2026-08-17).** Blok `TWO THINGS THAT NEVER CHANGE` slibuje „každé čtení přichází z jiného úhlu", ale `angleIntro` je jen v `RP_SINGLE`; `RP_LIFE`/`RP_ASK`/`RP_KRIZ`/`RP_NORNS`/`RP_HORSESHOE`/`RP_YGGDRASIL` ho nemají (ověřeno i na složeném promptu). Veškerá práce s úhly v `RUNAR_DECISIONS.md` je o single a nikde není napsáno, že u spreadů být nemá. Rozhodnout: je to záměr (spready mají strukturu z pozic), nebo výpadek?
  - **Zkusil jsem to změřit 2026-08-17 a NEJDE to** — a to je výsledek, ne výmluva. Průměrná párová shoda na trigramech: norns 0,0025 vs single 0,0009 (2,79×), **jenže půlka proti půlce uvnitř single dá 0,0002 vs 0,0029 = 14× rozptyl**. Rozdíl mezi rameny je menší než šum uvnitř jednoho ramene. Příčina je v datech: 923 single, ale jen **23 norns a 0 islandských**. **Co to odemkne:** vygenerovat norns dávku ≥50 v obou řečech, pak metrika začne rozlišovat.
- [x] **UZAVŘENO 2026-08-17 (owner): NEDOPLŇOVAT.** Islandský blok MYNDIN nemá zákaz „přirovnání navršené na metaforu" (2026-08-17, hlas — owner/Cowork).** Anglická verze ho má, islandská ne, a žádný záznam o tom neexistuje. CODE ho nedoplnil schválně: v EN má ten jev **1 výskyt z 800** čtení. Zato islandská čtení jedou na `eins og` mnohem víc než anglická na `like/as if` — **33,1 % vs 6,0 % čtení · 4,90 vs 0,91 na 1000 slov** (přežilo půlku proti půlce). ⚠️ Není to čistý důkaz: `eins og` je běžný idiom i mimo přirovnání. Otázka pro hlas: **je ta míra přirovnání v islandštině v pořádku, nebo Rúnara ředí?** Podle odpovědi buď doplnit pravidlo (a ověřit vazbu přes `is-vazba.py`), nebo zapsat, že IS ho mít nemá.
- [x] **UZAVŘENO 2026-08-17 — zámek zbytečný: `runar_character` sloupec `grammar` NEMÁ a tabulka je prázdná (0 řádků, ověřeno `supabase db query --linked`); žádný kód do ní nezapisuje. Jediná cesta byla shrine `{...DEF_CHAR}` a ta je zalepená. Hlídá to smoke ㉙ (`test_spine.js`).** Původní otázka: smí uložená postava přepsat islandský gramatický blok? Vlastní postava z `runar_character` přebíjí jazykový výchozí pole po poli — včetně `grammar`. Uložená anglická postava tak z islandského promptu udělá anglický (změřeno na shrine: 3780 vs 4855 znaků, blok gramatiky pryč, místo něj „Respond only in English"). Argument pro zámek: gramatika není rys povahy, ale 🔒 externě ukotvená islandština (§2). Souvisí: **owner ať zkontroluje, jestli v `runar_character` je aktivní řádek a co v něm je** — ze stroje CODE to nezjistí.
  - **Otestováno 2026-08-17 osmi tvary řádku, obě řeči.** Nebezpečný je **jen řádek, který má v `grammar` skutečný text** (pak IS prompt spadne z 4855 na 3780 znaků, gramatika pryč, místo ní „Respond only in English"). Bezpečné jsou: prázdný řádek, jen `personality` (4712), cizí sloupce `id`/`active`/`created_at` (4696) i `grammar` = prázdný řetězec (4855) — merge přeskakuje prázdné hodnoty (`runar-character.js:1105`). ⚠️ **Je to symetrické:** islandský řádek rozbije stejně tak ANGLICKÝ prompt. Uložená postava je jazykově slepá. → **Zámek by tedy stačil na JEDNO pole `grammar`**, ne na celou postavu.
...
- [x] **`RP_SINGLE.*.noqBranch` si odporuje se zbytkem promptu** — VYŘEŠENO při systémovém průchodu 2026-08-22 (dnes: „Mention X by name once, woven naturally", žádné „Let…" otevření runou; ověřeno v kódu). Původně: (nalezeno 2026-08-20 při měření „pojmenuj runu"): řádka začíná *„Let the quality of X arrive through image, **not explanation**"* / *„Láttu táknræn gæði X koma fram í myndum, **ekki útskýringu**"*, zatímco týž prompt o kus dál žádá `Mention X by name once`. Model tedy dostává dva pokyny proti sobě a rozhoduje se sám — a **rozhoduje se jinak podle jazyka**: s novým pravidlem „pojmenuj + uvaž obraz" anglický model starou větu přebil (6/8), islandský ji poslechl (4/20). Po smazání té jediné věty IS 8/20, EN beze změny — takže ta věta brzdí jen islandštinu, tedy primární jazyk (§2). ⚠️ Platí i BEZ nového pravidla: „zmiň runu jménem, ale nevysvětluj" je rozpor sám o sobě. Měření + čtení vedle sebe → artifact „Most mezi runou a obrazem" (2026-08-20). Rozhodnout: přepsat řádku tak, aby prompt říkal jednu věc.
- [ ] **Mapa nosičů: ke každému §1–§28 dopsat, která kontrola ho drží** (2026-08-21). Doloženo dvakrát týž den: pravidlo o bumpu  i pravidlo o délce doců existovala jako próza a obě byla porušena — první pěti migracemi, druhé devíti docy přes limit. Pravidlo bez nosiče je přání. Nosič je jeden ze tří: kontrola ve smoke · odmítnutí v okamžiku činu · výjimka s datem, která vyprší. Co nedrží nic, označit jako **nevynutitelné + proč**, ať na to nikdo nespoléhá. ⚠️ Mapa patří do SKRIPTU, ne do  — ten se needituje kvůli tomuhle (KUKY 2026-08-21).
- [x] **Zastaralý komentář u Yggdrasil builderů** — opraveno ve v4.13 patchi (2026-08-23):
  komentář teď nese kánon (kdykoliv/kdokoliv přihlášený; vrstvy V5 nečasově).
- [ ] **㉜ registr pravidel sbírá pevné řádky jen ze single cesty** (nalezeno 2026-08-23 při
  v4.13): spread-only instrukce (RP_YGGDRASIL.landing, beats spreadů) nemají otisk — nová
  protichůdná dvojice ve spreadech by prošla bez červené. Zvážit rozšíření sběru
  v `verify_prompt_rules_registry.js` na jednu postavenou cestu od každého spreadu.
- [x] **„Prázdné řádky v deníku" — UZAVŘENO: NENÍ BUG, byla to chyba měření** (2026-08-23).
  Řádky Berkana 18:10 a Jera 18:18 nesou KOMPLETNÍ čtení (340/301 znaků, uzavřená, usage zapsán) —
  CODE-tune četl `deep_text`, který je u singlu VŽDY prázdný (single bydlí v `short_text`).
  Stop v UI na server nedosáhne: proxy volání dokončí, odečte až PO úspěšném textu a čtení uloží
  CELÉ (invariant charged ⇔ journaled) — uživatel o zaplacené čtení nepřijde, najde ho v deníku.
  Kredit při stopu: rune_seeker ANO odečte (správně — neodečíst = start+stop dává Claude zdarma,
  serverový náklad běží tak jako tak); owner-admin metering přeskakuje (`!isAdmin` v proxy;
  ledger 23.8. prázdný, ověřeno). Volitelný nice-to-have: po stopu toast „čtení najdeš v deníku".
- [ ] **TESTY „recept: obrazy se potkávají" (KUKY 2026-08-23) — k otestování; generace CODE, slepé hodnocení Cowork.**
...
- [x] **Blank: jen 2 obrazy a jeden je abstraktní → model si maluje sám (voda)** — HOTOVO 2026-08-22: owner schválil, abstraktní řádek → nýfallinn snjór, přidán handfæri (Blank 3 obrazy); zároveň kajka za druhé „první kroky" jehně. Původně: (nález
  ...
  vodní hladinu (shrine ř. 317+323). Voda v runes.js Blank NENÍ — není to vlastnost runy.
  Motiv-guard s tím nesouvisí (sáček per runa; u 2řádkové banky se točí rychle). NÁVRHY
  ověřené (slepý test EN 3/3, IS ≥2/3, aspekty 3/3): nahradit abstraktní řádek „Nýfallinn
...
- [x] **Yggdrasil V5 — NASAZENO v4.12-mynd 2026-08-23** (9 významů Cowork #45, IS ověřeno, slepě EN 8/9 · IS 7/9, Norny z hlaviček ustoupily). Zbývá: oko ownera na celém čtení + živý provoz. — PŘERÁMOVÁNO 2026-08-23 (handoff #37: otázka „appka vs. doc" byla špatně položená — owner chce nejlepší možné významy, ne reconciliaci dvou starých). Čeká: owner potvrdí Coworku zadání → návrh → schválení → CODE-tune nasadí s golden+měřením. Původní rozchod (5/9 světů): (Cowork handoff #35 + ověření 2026-08-23): doc měl Hel=kořeny/předci/minulost, kód má Hel=completion/fulfilling itself · Vanaheimr příroda/intuice × harmony/balance · Álfheimr vědomé/jasné × creativity/kindling · Níðavellir řemeslo × deep source (řemeslo kód dává Svartálfaheimru) · Niflheimr potlačené × origin/unresolved. Opis z docu odstraněn (§20, poznámka doplněna) — původní znění v git historii RUNAR_DESIGN.md. Owner určí kánon; pak případná oprava RP_YGGDRASIL packů (EN+IS) jedním měřeným krokem.
...
- [ ] **Vymyšlený druhý člověk ve čtení („someone waits for the sound of your step")** (nález
  ...
  („the world is not arranging itself around them: no omen, no preparation") a NEkryje ji
  věta domény „Do not tell them what is true between them and anyone" (ta chrání vztah, ne
  vymyšlení celé osoby). Osa rizika: netrefa o TŘETÍ osobě může UBLÍŽIT (sám, po rozchodu),
  ne jen minout. Frekvence i jistota teď nízké (+2/16 v obou řečích, šum) → SLEDOVAT přes
  testování (profsteinn), verdikt až z většího n. ⚠️ Při případné opravě NEZTRATIT rejstřík:
  je to vinětka/scéna, ne diagnóza — moc ostrý zákaz zabije i to, co se na tom líbí
  (Cowork). Oprava = A/B přepis (znění oblasti či coldread půlky 2), ne přisypaná klauzule.
- [x] **„blind side" v `_registerContext` (Confirmation, EN+IS) — personifikující idiom** — HOTOVO 2026-08-22: otestováno A/B/C dle předpisu (n=8/rameno/jazyk, zamčený zbytek), vyhrál B „out of frame" (unaware 6/16 → 0/16), nasazeno ve v4.5-mynd, data docs/eval/2026-08-22-registr-ab/. Původně: (handoff Cowork 2026-08-22): bezpečný podmět, ale idiom skoro vždy o povědomí osoby — táž rodina jako opravené „beneath the surface" v úhlu. Tři návrhy přepisu v handoffu (A prostorový · B kotvený v obraze · C minimální), IS znění NEOVĚŘENÁ. ⚠️ Blok „co hledám" je od 2026-08-22 K LEDU (rozhodnutí ownera — ladí se čtení samo); řešit AŽ při jeho návratu, testem přes atribuci (zamčený zbytek, n≥8, oba jazyky), ne rovnou nasazením. Metodická přidaná hodnota hned: test „personifikující idiom" — použil-li by se výraz o člověku, znamenal by skoro vždy „něco, co o sobě neví"? — přidat vedle čáry podmětu při psaní příštích řádek promptu.
...
- [ ] **Server-side cena čtení (plná oprava metering)** — cenu odvodit z TYPU čtení na serveru, ne z čísla od klienta (dnes sanitizace 1..9). Chce: typ spreadu v requestu + mapa kind→cena v proxy + §13 full-path. Zapsáno 2026-08-22 při opravě NaN.
...
- [~] **Evidence pohybu kreditu (ledger)** — POLOŽKA BYLA ZASTARALÁ: ledger EXISTUJE od 2026-07-19 (trigger na user_profiles, append-only; odhaleno 22. 8., když ownerův běh mého duplikátního SQL narazil na existující tabulku — 29 řádků od 4. 8.). Zbývá FÁZE 2 = atribuce: `sql/2026-08-22_ledger_faze2_attribution.sql` (čeká na spuštění ownerem) + proxy posílá p_reason/p_ref (hotovo, deploy AŽ PO SQL). Pak ověřit: čtení za kredit → řádek s reason=reading a ref=id čtení. Původně: dnes NEEXISTUJE, takze na otazku "strhl se mi kredit?" nejde odpovedet ani u vlastniho uctu; `credits_balance` ukazuje jen aktualni stav, ne historii. Narazil na to owner 2026-07-19 pri prvnim zdarma cteni zivotni runy. Resi ZAROVEN starsi ownerovo zadani (2026-07-16): "musi byt za pouziti kodu, ten kod by se mel nekde zapsat. neni kod neni kredit. nikdy to nebude vic nez prodavame." **Podminka: zapis do evidence musi byt ATOMICKY se zmenou zustatku** — evidence, ktera muze chybet, lze, a lziva evidence je horsi nez zadna, protoze se podle ni rozhoduje.
...
- [x] **„already/þegar" — zadání přepsáno a POKRYTO 23. 8.:** slovo není vada (IS „þegar" = spojka „když"; EN „already" ve scéně je v pořádku — „fruit is already forming"). Vada je VZOREC „already known/deciding (somewhere beneath you)" = skryté vědění/predestinace — a ten už jmenují podtypy soudce chladu (profsteinn) a hlídá ho testování + no-recur. Samostatná páka se nestaví. Původně: Původní znění mířilo na *odstranění* toho slova. KUKY 2026-08-10: *„nejde nám o to zbavit se například `already` úplně. To byla chyba a nedorozumění. Chceme mít čtení vyvážená. Nejdeme hardcore zákaz na 0."* Prompt pravidlo (`_noColdRead`) míří na POHYB, ne na slovo, a to je správně — zůstává. Co zbývá: sledovat výskyt jako **ukazatel vyváženosti** (`measure_readings.js --balance`), ne jako počet prohřešků, a nestanovovat cílovou hodnotu.
- [ ] **IS ban-list (D-16)** — G1 v IS je neměřitelné (EN ban-list existuje, IS ne). Adaptovat EN + islandský kýč do IS grammar bloku.
- [x] **No-recur regresní sada** — POSTAVENA 2026-08-23: `scripts/verify_no_recur.py` (BAD_PATTERNS z check-is + fixed „selection" reporty → uložená čtení v DB [30denní okno = červená, starší = info] + commitnuté dávky [žlutá s místem]). Kalibrováno: květnové výskyty nehlásí jako návrat. Pouštět na požádání + před vydáním (síť, není ve smoke). Původně: — opravená model-output fráze se VRACÍ („Sittu með": F-001 ✗ · F-005 ✓ · F-006 ✗). Musí běžet nad OUTPUTEM (is-grammar-qa plocha), NE `check-is` (= source linter). Korpus už existuje (server-side save).
...
- [x] **Mytologické obrazy** — ZAVŘENO 2026-08-22 (KUKY: NE). Duplikát položky výše — obě zavřeny týmž rozhodnutím.
- [ ] **Shrine audit** — KUKY 2026-08-22: JE důležitý, zůstává. Chybí rozsah — definovat, co auditovat (přístupy? zobrazená data? edge fn?).
- [x] **Sigil Studio v0** — UZAVŘENO 2026-08-22 (KUKY: „sigil tady nemá co dělat, celkově nemá být v Rúnar"). V `v2/` žádný sigil kód není (ověřeno grep), nic k mazání.
...
- [x] **~~Tenké runy v bance obrazů~~ → HOTOVO 2026-09-24 (v4.50, DECISIONS 2026-09-24 (3)): žádná runa pod 3 obrazy v žádném období.** (změřeno 2026-09-23 po v4.49): od 1. 10. (říjen–únor) mají **Uruz, Berkana, Ehwaz, Ingwaz** jen 2 dosažitelné obrazy; **Dagaz a Blank** mají 2 od března do září. Se dvěma obrazy se čtení střídá A-B-A-B → přesně report „stejné čtení jako minule“ (Tiwaz/Thurisaz v září). Dopsat 2 obrazy na runu z ownerových popisů, brána s popisy. (CODE-tune)
- [ ] **Statické čtení Dagaz (Kolekce, s hlasem) odporuje ownerovu popisu run** — EN *„This is not gradual change — this is the breakthrough"*, IS *„umbreytir öllu í einu andartaki"*, kdežto `RUNAR_POPISY_RUN.md` říká, že se nic nestalo jedním velkým pohybem. Nový text runy pod ním (2026-09-24) drží popis, takže si teď na jedné obrazovce odporují. Přegenerovat statické čtení Dagaz (tabulka `runar_static_audio`, EL hlas) — rozhoduje owner. Nalezl kritik textů do Kolekce 2026-09-24. (CODE-tune)
  - **Owner 2026-09-24:** „nový Dagaz zní dobře, ale statická čtení zatím měnit nebudeme — necháme to na později.“ Schválené znění (ověřené is-grammar-qa + korpusem):
    EN: *Dagaz lives at the border between night and day. It rarely comes as a blaze. More often the same thing simply turns, and you see it from its other side. Then the world no longer looks quite as it did. Perhaps there is no new door to look for. Perhaps you already stand on its threshold.*
    IS: *Dagaz býr á mörkunum milli nætur og dags. Hún kemur sjaldnast eins og leiftur. Oftar snýst sami hluturinn einfaldlega við, og þú sérð hann frá hinni hliðinni. Heimurinn lítur þá ekki lengur alveg eins út og áður. Kannski þarf ekki að leita að nýjum dyrum. Kannski stendur þú nú þegar á þröskuldinum.*
- [ ] **Obraz se může zopakovat napříč zařízeními** (dnes sáček i „poslední obraz“ žijí v localStorage = per zařízení). **Doklad:** KUKY report 2026-09-23 18:42 *„podruhé za sebou rebarbora“* — Berkana 22. 9. 14:24 a 23. 9. 18:39 (UTC), obě s rebarborou. Oprava „nikdy tentýž obraz hned po sobě“ šla ven 18:36 UTC, druhé čtení přišlo o 3 minuty později — skoro jistě ještě na starém kódu (nasazení + cache); a i po opravě platí jen na jednom zařízení. V září má Berkana 5 kandidátů, takže z jiného zařízení je to 1 : 5. **Levné řešení bez serveru:** před losem načíst poslední čtení téže runy z journalu (`readings.prompt_draws` nese obraz) a vyřadit ho stejnou cestou jako `seasonlast_` (`_seasonBagPick(…, exclude)`). Jen pro přihlášené; návštěvník zůstává u localStorage. (CODE-tune 2026-09-23)
...
- [x] **UZAVŘENO 2026-08-20 — vyřešeno už 2026-08-16, jen to nikdo nezaškrtl.** Úhel [5] dnes zní *„Open at the edge of the image, where one thing turns into another"* — slovo „stirring" ani vazba „this rune" v `v2/runar-utils.js` nejsou (ověřeno ve zdroji). Definitivní přepis všech sedmi úhlů „doména → vstup do obrazu" = `e208a3a` + `3d55f94`; citace v tomhle řádku byla z mezikroku 2026-08-13. Nález Cowork, ověřil CODE-tune. Původně: **Úhel [5] „what is stirring" má tutéž vadu, kterou Cowork právě odstranil z [0] a [1]** — znění *„Lead with what is stirring — name the movement this rune makes visible"* / *„Byrjaðu á því sem hrærist — nefndu hreyfinguna sem þessi rúna gerir sýnilega"* se ptá **na runu**, ne na život leitanda. V dávce 2026-08-09 vyrobil 2/2 definic (EN) a 2/2 papouškování obrazu (IS) — n=2, takže **hypotéza, ne nález**; strukturní argument ale platí bez ohledu na n. LANE: obsah = Cowork, CODE jen zadrátuje a změří. Změřit jde teď vynuceným úhlem: `node scripts/utils/gen_batch.js --angle 5` (n≥25 staré znění proti novému).
...
- [x] **🐞 `rworld()` má jen ANGLICKÉ popisky — UŽ NEPLATÍ** (ověřeno produkční cestou 23. 8.: DREGNA RÚNA řádek je islandský; zbývala jen sesterská vada elementů — opravena v4.6, relements s IS mapou). Původně: — islandský prompt nese angličtinu** (nalezeno `lint_prompts.js` 2026-08-13). `v2/runar-utils.js` mapuje Midgard → „the living moment, what is active now" a stejně tak Hel/Asgard/Vanaheim/Jotunheim, bez jazykové větve. V IS promptu se ta anglická fráze objeví **dvakrát** — v řádce `DREGNA RÚNA` a v otevírací větvi. Porušuje §2 (IS je primární, ne překlad). Potřebuje pět islandských popisků → **obsah = Cowork**, zadrátování + golden = CODE.
- [x] **Popis světa je v promptu dvakrát — UŽ NEPLATÍ** (ověřeno 23. 8.: 1 výskyt v produkčním promptu; oprava přišla 13. 8. — noqBranch svět nevypisuje). Původně: (tentýž nález, druhá vada): `rworld()` se vkládá do `DRAWN RUNE:` i do otevírací větve `Open with X — let its quality (…)`. Sedm slov navíc v každém čtení, v obou jazycích. Rozhodnout, která z těch dvou zůstane.
- [x] **IS: pokyn o rodu je v promptu dvakrát — UŽ NEPLATÍ** (ověřeno 23. 8.: grammar pravidlo 5 dnes zní „Kynið er tilgreint í ÁVARP; fylgdu því" — odkaz, ne duplikát). Původně: — `DEF_CHAR_IS.grammar` pravidlo 5 („Samræmdu ÖLL lýsingarorð og fornöfn um þú") a `_addressContext` (ÁVARP) říkají totéž. Systémový prompt to navíc říká obecně, čtecí konkrétně — pravděpodobně stačí ten druhý.
- [x] **`hooks/pre-commit.py` nechává v indexu STAROU verzi `sw.js`** — VYŘEŠENO 2026-08-22: `hooks/post-commit.py` index sám srovná + smoke ㉞ minu hlídá. Původně: — po `git commit -F msg -- <cesty>` (pathspec, bez `v2/sw.js`) skončí HEAD i pracovní strom na nové verzi, ale **index na předchozí** (pozorováno 2026-08-09: HEAD/working v286, index v285; a před tím tam ležel v284 z jiné session). Je to přesně ten stav, ze kterého 2026-08-02 vznikl produkční downgrade v249→v248 — downgrade guard v hooku ho chytí, ale vzniká pokaždé znovu. Zatím se uklízí ručně: `git restore --staged v2/sw.js` (obsah souboru nemění).
...
⭐ **ROZHODNUTO 2026-08-16 (KUKY):** *„teď je repo v pohodě, jelikož není produkce. Až bude
...
- [ ] **MOŽNOST: cestu pro rameno píše UŽIVATEL — do Asku, vlastními slovy** (KUKY 2026-09-12).
  ...
  **Ukázková čtení (CODE 2026-09-12, ~85 slov, NEsouzeno):**
  ...
  ⚠️ **KUKY 2026-09-12:** *„nechat otázku na uživateli je záludné, jelikož potřebuje vědět, jak se zeptat.
  Uvidíme, kam nás to posune."*
...
- [ ] **PŘIPRAVENO K PUŠTĚNÍ: čtení + Ask „jaký efekt má moje životní runa na tuhle runu"** (KUKY 2026-09-12, odloženo na příští session — „ty čtení uděláme, až se resetuje"). Nic se nemusí skládat znovu, setup je celý tady.
  ...
  **SADA — 7 run, po jedné na každou osu. BEZ OBRAZU, jen ukotvení** (KUKY 2026-09-12: *„ale já to nechci s obrazem, já to chci bez obrazu s ukotvením"*). ⚠️ Do promptu tedy NEJDE nic z `RUNE_IMAGES` — jediný vstup je runa + její osa. Důvod: v dnešní zkoušce byly bezobrazové verze *„o cestě"* a nesly se z ramene na rameno, kdežto nábytkový obraz (káva, klíče) se musí postavit v každém čtení znovu → `RUNAR_DESIGN.md`, „Co je OBRAZ — a dva jeho druhy".
  ⚠️ **Gebo v sadě NENÍ schválně** — je to ownerova životní runa a `_lifeWasDrawn` by čočku shodil.

  | osa | runa | ukotvení = JEDINÝ vstup do promptu |
  |---|---|---|
  | ① poloha | **Raidho** | jdu, a v jakém tempu |
  | ② terén | **Hagalaz** | počasí, které cestu přepíše |
  | ③ viditelnost | **Perth** | něco je zakryté a ještě se neukázalo |
  | ④ co nesu | **Fehu** | zásoby, které se cestou spotřebovávají |
  | ⑤ kdo jde se mnou | **Ehwaz** | ten, kdo mě nese, a důvěra v něj |
  | ⑥ co roste pod cestou | **Berkana** | něco začalo a roste, ať jdu kamkoli |
  | ⑦ cesta ještě není | **Blank** | nikdo tudy ještě nešel |

  **POSTUP:** ke každé runě (a) čtení ramene z ukotvení, (b) Ask s otázkou **„how does my life rune Gebo affect this rune?"** — znění doslova to nasazené (`ask_hint_life`), ať jde srovnat s produkcí. Čtení píše CODE v konverzaci (memory `cteni-generuj-tady-ne-pres-api`), slepé souzení subagentem.
  **CO MĚŘIT:** umísťuje rameno člověka na cestě? · **zůstane text na cestě, nebo si přitáhne nábytek** (bez dodaného obrazu mu nic nebrání vymyslet si hrnek — Gebo-C 2026-09-12 si vyrobilo stopy v zemi, což je cestovní; nábytek by byl regres) · zavírá, nebo dává **tvar bez obsahu** (třetí stav → `RUNAR_EVAL_LOG.md` 2026-09-12) · a v Asku to hlavní: **chodí životní runa jako CHŮZE, nebo se z ní stane druhý význam vedle prvního?**
...
**Ověřeno CODE-read 2026-08-22 — fakta, ať je nikdo neověřuje potřetí:**
- ✅ **`tree_state` EXISTUJE a je zapojená** (11 sloupců; čte `claude-proxy`, zapisuje edge funkce
  `tree-update`). Neexistuje jen **`tree_readings`**. Coworkův handoff tvrdil, že chybí obě, protože
...
**Otevřené pro ownera:** cena/gating oblouku (jedna platba, nebo osm?) · pořadí odhalování ramen ·
zobrazuje se uživateli směrový label? · **a nadřazeně: Vegvísir jako symbol vůbec** — novověký
galdrastafur (Huld 1860), zároveň nejkomercializovanější islandský symbol; rozhodnutí o značce,
ne technické.

⚠️ **Kánonové riziko, které si návrh nepojmenoval:** symbol, jehož celý účel je *„ukázat cestu,
neztratit se"*, stojí proti pravidlu *„Rúnar nikdy neřekne krok, jen tvar"*. To je přesně třída,
kde jsme naměřili úniky **i s aktivními mantinely** (landvættir 2/4 → „jsi chráněn",
