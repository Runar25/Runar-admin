...
- [ ] **Čočka životní runy nedochází na velké spready (2026-08-18, nalezla smoke ㉚).** `_lensContext` přispívá u `single` a `norns`, ale u `kriz`, `horseshoe` ani `yggdrasil` ne — tam se tedy životní runa jako čočka neuplatní. Nikde není napsané, že to tak má být. Rozhodnout: záměr (velký spread má dost vlastní struktury), nebo mezera jako u ÁVARP? Do rozhodnutí vedeno jako výjimka v `scripts/verify_prompt_levers.js`.
