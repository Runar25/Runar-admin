...
- [ ] **`landvættir→Algiz` a `hvalreki→Nauthiz`: při implementaci NESTAČÍ obecné mantinely (2026-08-19, od CODE-read).** Obě vazby protekly do kánonové čáry i s aktivními zákazy (cold reading / rada) — Algiz do „svět tě chrání", Nauthiz do pokynu tazateli. ⚠️ **Symboly v kódu zatím NEJSOU** (ověřeno grepem přes `v2/`), takže tohle je podmínka pro chvíli, až je někdo přidá do `RUNE_IMAGES`: (a) guardrail nesený **s fragmentem**, ne obecným pravidlem — u Algizu lokalizovat stráž **v tazateli** (vzor `kría`: „something in you"), u hvalreki výstraha jako **přísloví stojící vedle**, ne pokyn; (b) **cílený kánon-eval** na ty dvě vazby, ne jen obecné no-cold-reading. Vzor korpusu: `~/runar-eval/canon-check-en.txt` (mimo repo, ověřeno že existuje). Čísla a rozbor → `RUNAR_EVAL_LOG.md` 2026-08-19 — tady se neopisují. ⚠️ Evidenční hranice: **n = 4 na vazbu, 2/4** — na tvrzení o míře je to málo (viz laťka ≥20–25 v `memory/cowork-handoff-quality-bar.md`); nese to mechanismus, ne to číslo.
