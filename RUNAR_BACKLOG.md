...
- [x] **~~ČOČKA ŽIVOTNÍ RUNY — owner zvažuje pozastavení~~ → VYŘEŠENO 2026-09-10 volbou ano/ne** (`user_profiles.life_rune_in_readings`, default zapnuto; přepínač v bočním panelu). ⚠️ **Čeká na SQL od ownera** — `sql/2026-09-10_life_rune_in_readings.sql` + `sql/2026-07-16_user_profiles_column_grants.sql`. Rozhodnutí → `RUNAR_DECISIONS.md` 2026-09-10. Zbývá otevřené jen to, co owner zmiňoval jako alternativu: životní runa jako doplňující otázka v Ask (Ask je premium-only a `buildAskPrompt` k ní nedosáhne). Původní zadání: (KUKY 2026-09-10: *„čočka zabírá místo pro čtení… nejsem si jistý, jestli se hodí dávat ji automaticky"*). **Změřeno: bere si závěrečnou větu v 10/12 čtení** (bez ní 0/12); čtyři různé tažené runy, všech dvanáct závěrů o životní runě. Stojí +5 % délky (EL). Je to **třetí uchazeč o poslední větu** vedle losu tvarů konce. → `RUNAR_EVAL_LOG.md` 2026-09-10.
  **Tři cesty, co owner zvažuje, a co která stojí:**
  ...
  3. **Do Ask** — sedí s tím, že Ask je nejčistší plocha, ⚠️ ale **Ask je premium-only** → životní runa by se stala prémiovou funkcí; a `buildAskPrompt` k `u.lifeRune` nedosáhne (týž plumbing jako u seeking).
...
- [ ] **Motiv-guard běží skoro naslepo — 79 z 85 obrazů nemá motiv** (8. sloupec `RUNE_IMAGES`, nalezeno 2026-09-09). Guard má bránit tomu, aby týž pojmenovaný motiv přišel dvakrát po sobě (smoke ㉟), ale bez motivu ho nemá na čem uplatnit — u šesti obrazů z 85 funguje, u zbytku ne. Doplnit motivy. ⚠️ **Váže se to na návrh „předloha + losovaná rodina předmětů"**: bez plných motivů nejde uhlídat, aby dvě tažené runy nedostaly týž typ obrazu (obava ownera 2026-09-09).
...
- [ ] **MOŽNOST C — registr čtený z follow-up otázky místo z nabídky** (Cowork 2026-09-08, owner: *„necháme zatím jako možnost, pokud to ve čteních nebude vidět; ale bylo by dobré to alespoň otestovat, ať víme"*).
  ...
  **Co proti (CODE-tune, ověřeno v kódu):** Ask je **premium-only** (`mode === "ask" && userTier !== "premium"` v claude-proxy) → registr by se stal prémiovou funkcí · `buildAskPrompt(reading, question, runes, lang, corrections)` k `seeking` vůbec nedosáhne, je to plumbing, ne přeformulování · **není to buď-anebo** (Ask může číst registr z otázky a ve čtení může zůstat dvanáctislovný registr).
  **Trigger:** ⚠️ Ask **má** tři rotující nápovědy (`ask_placeholders`), ale **nemá řádku nápovědy nad polem** jako specifická otázka — a placeholder zmizí, jakmile člověk začne psát. Kdo bude C stavět, musí napřed rozhodnout, jestli má uživatele navádět (a tím registr *vyrábět*), nebo ne (a číst, co přijde samo). **Bez korpusu skutečných follow-up otázek se to změřit nedá** — to je první krok, a je to práce pro Cowork.
