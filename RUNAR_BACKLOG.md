...
- [ ] **Tři páky pestrosti, které v kódu jsou a do čtení nedojdou** (nález 2026-09-08, inventura + vlastní ověření; měření → `RUNAR_EVAL_LOG.md` 2026-09-08). Vrátit se k tomu, až se bude ladit hlas před spuštěním — **každá z nich je pestrost zadarmo, bez psaní nových obrazů**:
  ...
  2. **Sampling modelu je neřízený** — `temperature`/`top_p`/`top_k` se nenastavuje nikde. Je to největší jednotlivý zdroj rozdílu dvou čtení z téhož promptu a jede na defaultu, který neznáme. Zároveň to je důvod, proč **nejde golden-verify na text** (bez seedu). Rozhodnout, jestli to chceme řídit.
  ...
  4. **Sowilo má jediného kandidáta v `deepwinter` i `darkening`** → od září do února dostává pořád týž obraz. Nejlevnější jednotlivá oprava z celého seznamu: dopsat Sowilu zimní kandidáty.
