...
- [ ] **🐞 Single čtení uložená s PRÁZDNÝM textem (v4.12, 2026-08-23 18:10 Berkana + 18:18 Jera, mode mine)** — short_text i deep_text = prázdný řetězec (ne NULL), prompt_draws uložené, kredit nestržen. Model text vrátil (proxy prázdno odmítá PŘED žurnálem), ale serverové `composeReading` dalo '' → podezření: výstup modelu v tvaru, který compose mirror nezvládá (⑦ hlídá paritu se vzorky, ne s tímhle případem — §19.3 jiná plocha?). K diagnóze: (a) ověřit u ownera, zda čtení NA OBRAZOVCE text měla (klient parsuje nezávisle → určí client-vs-server), (b) repro: prohnat composeReading výstupy s fences/prefixy, (c) zvážit guard v persistJournal: composeReading('' z neprázdného textu) → uložit raw text + log, nikdy tiché prázdno.
