...
- [ ] **Úhel čtení má JEN `single`, ostatních šest cest ne (2026-08-17, produkt — owner).** Blok `TWO THINGS THAT NEVER CHANGE` slibuje „každé čtení přichází z jiného úhlu", ale `angleIntro` je jen v `RP_SINGLE`; `RP_LIFE`/`RP_ASK`/`RP_KRIZ`/`RP_NORNS`/`RP_HORSESHOE`/`RP_YGGDRASIL` ho nemají (ověřeno i na složeném promptu). Veškerá práce s úhly v `RUNAR_DECISIONS.md` je o single a nikde není napsáno, že u spreadů být nemá. Rozhodnout: je to záměr (spready mají strukturu z pozic), nebo výpadek?
