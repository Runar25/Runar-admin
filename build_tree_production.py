# -*- coding: utf-8 -*-
# build_tree_production.py — generates v2/runar-tree-prod.js
#
# Production tree render module. PORTS the crown-composer composition
# (build_crown_composer.py, reading-driven) into a reusable, UI-free module:
#   RunarTreeProd.render(canvas, { log, rune, dob, skin })
# where log = [{spread, runes:[{rune,el}], area, intention}], rune = life-rune key,
# dob = {d,m,y}. Consumes window.RunarTrunk + window.RunarBranch + global RUNES.
#
# Composition (paintLimb / growBranch / routingFromLog / stableAssign / emergence /
# exitIndex + orchestration) copied 1:1 from the lab (behavior identical). Dropped:
# sliders, localStorage, inspect/history, demo/routing() simulation, DOM panels.
# ANTI-DRIFT DEBT (§18): lab + prod share this composition by copy. TODO factor a
# shared runar-tree-core.js both load. Logged in RUNAR_DECISIONS.md.
# Rule §1: JS is generated via this Python script (never hand-edit the .js).
import io, os, time

V2 = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'v2')

JS = r"""/* runar-tree-prod.js — GENERATED by build_tree_production.py (do not hand-edit).
...
p = os.path.join(V2, 'runar-tree-prod.js')
with io.open(p, 'w', encoding='utf-8', newline='\n') as f:
    f.write(JS)
print('written', p, len(JS), 'chars')
