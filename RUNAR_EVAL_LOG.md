...
### 2026-09-12 — ❌ ZBYTEČNÝ TEST: vymyslel jsem pičovinu, kterou nikdo nechtěl (CODE-read)

**Owner:** *„nic to pro Vegvísir neznamená" · „Vegvísir má určitou strukturu, ty jsi ji úplně ignoroval
a šel cestou, která mě nezajímá, protože jsem věděl, že to takhle dopadne" · „tokeny i čas pryč".*

**Co jsem udělal:** 48 slepých pisatelů + soudci (~2M tokenů) nad **produkčním** Ask promptem
(produkční pravidla + `_askLifeContext`), otázky převzaté z produkce — včetně otázky na práci.

**Proč je to špatně:**
1. **Vegvísir není produkční Ask.** Má vlastní strukturu (RÁMEC a varianty v `RUNAR_BACKLOG.md`, sekce
   Vegvísir: střed = životní runa jako chůze ve variantách, rameno vstupuje otázkou). Test ji celou
   vynechal — žádná varianta chůze, a pravidla, která pro Vegvísir neplatí.
2. **Otázka na práci** s cestou ani s runou ramene nemá nic společného.
3. **Rozsah:** že produkční pravidla scénu odmítají, bylo vidět na prvních pár textech. Stačilo ~5 agentů
   a škálovat až po potvrzení — a před velkou dávkou se zeptat.
...
čtení `global`). Nástroj → `node scripts/utils/stats.js` sekce „náklady" a „cache" (samotest výpočtu; model bez ceny nahlásí).
  ⚠️ Do téhož dne tu stál samostatný `scripts/cena_cteni.js` — byl to druhý nástroj na totéž sledování; sloučen do `stats.js`  <!-- doc-links:ok 2026-09-24 záměrně: záznam, že soubor byl smazán a sloučen do stats.js -->
  (rozhodnutí 2026-08-15 „sběr dat před vizualizací": provoz a cena patří do jednoho přehledu), `cena_cteni.js` smazán.  <!-- doc-links:ok 2026-09-24 záměrně: záznam, že soubor byl smazán a sloučen do stats.js -->
