...
- **Security hardening (2026-07-17)**: pět reálných děr zavřeno + guard. **(1)** `user_profiles` RLS policy byla `auth.uid()=id` (řádkový filtr) ale `authenticated` měl tabulkový UPDATE grant → RLS nefiltruje SLOUPCE → každý přihlášený si z konzole PATCHnul `credits_balance`/`tier`/`month_units` (ověřeno naživo, status 200). Fix = sloupcové granty (`sql/2026-07-16_user_profiles_column_grants.sql`, klient píše 12 sloupců + insert jen `id`; peníze píše jen service_role). **(2)** `mode:'ceremonial'` = prázdná větev v proxy → čtení zdarma komukoli, kdo pošle ten client-string; Gathering-čtení je mrtvý kód (žádný caller, whispers-* UI v HTML neexistuje) → větev smazána. **(3)** `mode:'ask'` (moje chyba téhož dne) obcházel měsíční limit → teď vyžaduje ověřený rodičovský `readings` řádek (existuje, patří userovi, `follow_up` prázdný) = 1 follow-up na 1 započítané čtení. **(4)** `elevenlabs-static` = 0 auth přes „Admin only" komentář → admin JWT gate + shrine posílá token. **(5)** clampy: claude-proxy `max_tokens`≤2500, elevenlabs text≤3000 zn. **Mincovna `gift_codes` OVĚŘENA bezpečná** (RLS zapnuté + policy `auth.email()=admin` → non-admin vidí 0 řádků; `has_table_privilege=true` je jen grant, RLS je brána). **Guard = smoke ⑩** `verify_write_surface.js` (klient vs granty — 3 způsoby červené, ověřeno rozbitím) + `sql/audit_write_surface.sql` (DB strana, owner pouští). Redeem-replay přes delete-account (uvolní `used_by`) = zbývá, ale potřebuje prodanou kartu (žádná není) = před launchem. Detail: RUNAR_DECISIONS.md 2026-07-17.
- **No-cold-read gate + follow-up gaty (2026-07-17, prompt v1.0)**: `_noColdRead(lang)` = JEDEN sdílený gate ve **všech 5 builderech + `buildAskPrompt`**; follow-up dostal navíc `_describeRule` (neměl ho) + **anti-mirror**; RP_ASK už neříká „deepen what the runes **already** said". Vada není slovo, ale **tah** (nevyvratitelné „už je to v tobě" / „svět to připravoval" = G2b). **Task 1a NEUDĚLÁN — mrtvý kód**: „already" angl je v `READING_ASPECTS` (`_randomAspect()` bez volajícího); živý `READING_ANGLES` „already" nemá → přeformulování by nezměnilo nic; proto zapojeno i do hlavního těla. **Task 3 NEUDĚLÁN — mylná premisa**: hlavička už IS název ukazuje (`rn()` → `is_n`), Coworkovo „tahá latinské n" neplatí. **KUKY rozhodl 2026-07-17: závorková glosa ZŮSTÁVÁ** („PERÞ (DULDIR HLUTIR)") → uzavřeno bez změny kódu. Smoke ⑧ rozšířen (staví reálný ask prompt; ověřeno rozbitím). IS: E001 vyřešen přepisem na plné věty, ne odložením na Sigrún. **Efekt NEZMĚŘEN** — čeká na v1.0 kohortu vs v0.9/baseline. Detail: RUNAR_DECISIONS.md 2026-07-17.
...
- [handoff-text-in-code-block.md](handoff-text-in-code-block.md) — text k předání jiné session VŽDY do code blocku (copy jedním klikem), ne do prózy
