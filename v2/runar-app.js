...
async function generateLifeRuneReading() {
  ...
  if (loadTxt) loadTxt.textContent = isIs ? 'STEINARNIR TALA Í ÞAGNINNI…' : 'THE STONES SPEAK IN SILENCE…';
  ...
  if (res.error) {
    if (loadTxt) { loadTxt.textContent = isIs ? 'Villa við lestur.' : 'Error generating reading.'; loadEl.style.display='block'; }
    return;
  }
