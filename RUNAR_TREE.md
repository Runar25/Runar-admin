...
- ⭐ **PROTI PALMĚ: podlaha výstupu větví je posuvník, ne tvrdý ořez (2026-08-18, KUKY: *„vypadá to spíš jako palma"*).** Dřív `Math.max(0.50, start − pořadí·krok)` — tvrdá podlaha se dvěma následky: pod polovinou kmene nevyrostlo nic (dlouhý holý kmen) a od 8. větve se všechny slepily na jedné výšce. Nově se řada k podlaze jen **blíží** (pevný podíl na krok), takže žádné dvě větve nesplynou, a `exitFloor` říká, jak nízko strom větví. Změřeno na 12 větvích — slepených **5 → 0**; nejnižší větev: podlaha 0,50 → 53 % · 0,30 → 35 % · 0,12 → **19 %**. Při výchozí 0,50 sedí prvních pět větví na týchž výškách co dřív (96 · 88 · 85 · 78 · 71), takže se výchozí vzhled nehne skokem. ⚠️ **Sestup musí být pevný podíl na krok, ne podíl ze zbývajícího rozsahu** — první verze dělila krok rozsahem, snížení podlahy rozsah zvětšilo, tím zpomalilo sestup a páka si sama sežrala účinek (0,50 → 0,12 posunulo nejnižší větev jen 54 % → 40 %). ⚠️ **Podlaha není celá odpověď:** na logu s 6 větvemi je i při 0,15 nejnižší větev na 50 %, protože větví je prostě málo — zbytek palmy je v jejich počtu, ne v podlaze.
...
- 🐞 **DO GENEROVANÉHO SOUBORU SE NESAHÁ (2026-08-18).** `runar-branch.js` a `runar-tree-prod.js` vyrábí Python builder; kdo patchne rovnou `.js`, vyrobí časovanou bombu — všechno šlape, dokud někdo nespustí build, a pak změna tiše zmizí. Nalezeno na sobě: `subLenMul` (posuvník *délka výběžků* u kořenů) žil jen ve vygenerovaném `.js`, `eWob`/`eTip` odvozené z `curve` taky — generátor tedy zabíjel při každém běhu jednu nebo druhou, podle toho, kdo ho spustil. Obojí doplněno do `build_branch_composer.py`. **Ověření, které to prokáže:** po regeneraci musí být diff proti gitu prázdný, a druhý běh za sebou taky — „vypadá stejně" se nepočítá.
...
- ⭐ **REŽIM „WebGL kůra" (2026-08-10, KUKY).** Třetí tlačítko v REŽIM vedle *kůže* a *kostra*. Strom jako **pás trojúhelníků s UV**: `u` běží podél větve (ujetá délka), `v` napříč — takže textura kopíruje **povrch**, ne obrazovku. To je jediný, ale rozhodující rozdíl proti 2D, kde se dlaždice četla jako nálepka. GL plátno leží nad 2D s `pointer-events:none`; 2D si nechává čáru země a zlaté zvýraznění, takže **výběr větve funguje i v GL**. Posuvníky `glZrno` (34) a `glHloubka` (0,85).
  - 🐞 **Tři vady nalezené měřením při stavbě:** (1) **vlákna kůry běžela napříč větví** — v dlaždici jsem je protáhl osou `v` místo `u`, tedy tatáž vada jako u 2D verze. (2) **`v` bylo natvrdo 0→1 přes šířku**, takže přes šířku byla vždy přesně jedna dlaždice a posuvník měřítka napříč nedělal nic (rozptyl 18,5 → 19,0 = nula); nově je UV **izotropní**, `v` taky v pixelech / měřítko (16,2 / 14,1 / 14,6). (3) **Tenké větvičky mizely** — sub-pixelové trojúhelníky se nerasterizují, obrys byl o 84 px užší; minimální šířka to stáhla na −36 px.
  - **Ověřeno:** osa kmene sedí s 2D na **0,4–0,6 px** (dvě ze tří výšek; třetí je u země, kde 2D kreslí kořenové výběžky jinak). Žádná chyba v konzoli.
- 🐞 **Pryč s „příčnými prasklinami" (2026-08-10, KUKY: *„co to má za smysl vytvářet pravidelné čáry na kmeni?"*).** Generovaly se **na každý segment** (`Math.floor(a.t*40)+j`), takže vycházely v pravidelném rytmu podél kmene — pravidelnost je vada, ne kresba. Odstraněno.
- ⭐ **KŮRA ZE STAVBY, ne z dlaždice (cesta 3, 2026-08-10, KUKY).** Kmen už JE svazek pramenů; kůra vzniká tím, že prameny **jsou vidět** — přesně tak vzniká i na skutečném stromě (podélné hřebeny a rýhy mezi nimi). Dvě páky: **`tonPramene`** (každý pramen o kousek jinak tmavý — bez toho splynou v jedno tělo, protože při rozestupu 0,5 px není co odlišit) a **`hrebeny`** (tmavá rýcha po stíněné straně + osvětlený hřeben). ⚠️ **Kreslí se JEDNÍM PÁSEM přes celý pramen** (`ribbon()`, šířka z lokální tloušťky) — kreslení po segmentech bylo příčinou těch příčných pruhů, kvůli kterých se vracela dlaždice. Škáluje se samo: při 1× jemné zrno, po zoomu skutečné hřebeny; žádný práh.
  - 🐞 **Pásy se nejdřív kreslily PŘED tělem, takže je tělo přemalovalo** — posuvník měnil 5 188 px (průměr 17). Po přesunu za tělo **14 169 px (průměr 49)**, tedy 3×. `tonPramene` byl silný od začátku (29 275 px, průměr 89). Cena +7 % (87 → 93 ms).
  - ⚠️ **Mez, kterou to nepřekročí (změřeno předem):** kmen je v modelu **8–14 px** široký, po auto-fitu ~4–6 px na obrazovce, prameny 0,5–1,6 px od sebe → **vejdou se 2–4 hřebeny**. Kůra jako kresba je čitelná až po zoomu. Fotorealistická kůra tudy nevede vůbec.
- ⚠️ **POKUS O PIXELOVOU TEXTURU KŮRY — VRÁCEN (2026-08-10).** Procedurální dlaždice (šum → fBm → hřebeny → domain warp) položená přes siluetu stromu jedním `multiply`. Technicky to fungovalo a bylo zadarmo (−3 % času), ale **vypadalo to špatně** (KUKY) — na kmeni vznikly příčné pruhy, strom četl jako článkovaná trubka. Kód odstraněn. ⭐ **Poučení, které zůstává: tohle se nemá zkoušet, dokud není rozmyšleno, JAK se to dělá** — viz rozbor v RUNAR_BACKLOG (textura potřebuje plochu, kterou 2D plátno nemá).
...
- ⭐ **PANEL KORUNY ROZDĚLEN PODLE DRUHU PÁKY (2026-08-18, KUKY: *„čím víc věcí, tím horší je to ladit… nevím, jestli vůbec má smysl to ladit"*).** Zmatek nedělal počet posuvníků, ale to, že panel míchal dva různé druhy. Nově tři sekce: **TVAR** (8 — rozhodují, co strom o člověku říká; tady ladění smysl má) · **VZHLED** (8 — doladí se jednou a zapeče) · **ODLOŽENO** (15, složené, nic se nemaže). Ověřeno: 31 posuvníků před i po, žádný ztracený ani zdvojený, odložené dál kreslí a `reset koruny` sahá na všechny tři.
  - **Jak se rozhodovalo:** každý posuvník projet min→max a změřeno, kolik bodů na plátně se změní. Rozpětí **10 855 → 0**. Nula ale neznamená mrtvý: **`pestrost cteni` je mrtvá vždy, když je log** (používá ji jen demo strom bez čtení), a **tři WebGL páky žijí jen v režimu „WebGL kura"** — v `kuze` se měří nula právem.
  - ⚠️ **`odbocky (twigy)` (`childN`) NENESOU runu** — dítě bez seznamu dostane `cRune = rune`, tedy runu rodiče. Je to ta ozdobná rekurze, co tvoří 63 % větviček: 30 % vizuálního účinku, nula informace. Popisek to teď říká přímo v labu.
  - **`runy schovane pod graduantem` (`kidsMax`) význam MÁ:** rozhoduje, kam se pověsí runa tažená až po graduaci — na hlavní větev (0), nebo pod graduanta (1+). Čte se jako „vyrostlo to z té praxe".
