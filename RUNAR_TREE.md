...
- 🐞 **MALÉ VĚTVIČKY NENESOU TAŽENOU RUNU — nesou runu SVÉ VĚTVE (nalezeno KUKYm 2026-08-10).** Po jednom Norns čtení má každá ze tří větví v tabulce `odb. 0`, ale ve stromě jí visí několik větviček. Změřeno proklikáním koruny: vracejí **„Berkano — ODBOČKA" na berkanové větvi, „Jera — ODBOČKA" na jerové** — tedy tutéž runu, na které rostou. Je to **dekorativní fraktální rekurze** v `growBranch` (posuvník `odbocky (twigy)`, `childN`, `maxDepth 3`): když větev nemá žádnou JINOU taženou runu svého elementu, `childRunes` je prázdné → kód spadne do dekorativní větve a dětem přiřadí `cRune = rune` rodiče. ⚠️ **Odporuje to dvěma věcem, které už platí:** pravidlu „větev vyroste jako klacík bez odboček a další přiroste až s dalším tažením té runy" (KUKY 2026-08-07) a zásadě „na každý výskyt ve stromě musíme mít odpověď proč". Je to táž vada, kvůli které se zrušily letorosty (F6) — jen o patro výš a přehlédnutá. **Rychlá páka:** `odbocky (twigy)` na 0 (ale sebere i drobné větvičky na SKUTEČNÝCH odbočkách). **Správná oprava** (nepostaveno): při reálném logu dekorativní zálohu na úrovni 0 nepouštět, nechat ji jen pro demo strom bez čtení.
