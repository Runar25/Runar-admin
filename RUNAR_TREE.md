...
- ⭐ **SKUTEČNÁ TEXTURA KŮRY (2026-08-10, KUKY: *„když říkám texturu, tak myslím texturu"*).** Vektorové čárky nahrazeny **procedurální dlaždicí**: periodický hodnotový šum → fBm → hřebenová funkce `1−|2n−1|` → **domain warp** druhým šumem (praskliny se vlní, rozdvojují a slévají místo rovných pruhů) + jemné zrno. Mřížka je **svisle protažená 8:2**, aby vznikly podélné praskliny, a **periodická**, takže dlaždice je bezešvá. Počítá se **jednou** a kešuje.
  - ⚠️ **APLIKACE MUSÍ BÝT JEDNÍM PRŮCHODEM.** První verze omotávala dlaždici kolem každého segmentu (ořez + `multiply` na segment): správný směr zrna podél větve, ale **1226 ms proti 248 ms = +394 %** — neunosné. Nyní: dlaždice se vykreslí do offscreenu v soustavě stromu, `destination-in` + obrázek plátna ji ořízne **siluetou stromu**, a jeden `multiply` ji položí na strom. Cena **−3 % (v šumu)**, nezávislá na počtu segmentů. Plátno se `clearRect`uje, takže maska funguje; pozadí mimo strom se neztmaví (ověřeno).
  - **Cena za to:** textura je v prostoru **plátna**, ne podél každé větve. Na kmeni (svislém) to sedí; na tenkých větvích směr zrna stejně nikdo nepřečte. Kdyby se zrno mělo ohýbat podle větví, stojí to zmíněných 5×.
  - 🐞 **Dvě vady vlastní první dlaždice, nalezené měřením:** (1) **obrácená polarita** — dlaždice byla většinou tmavá, takže `multiply` ztmavil celý kmen a **zploštil ho** (rozptyl jasu v kmeni klesl 30 → 17 místo aby stoupl). Musí být většinou **bílá**, aby se ztmavily jen praskliny. (2) **Měřítko 20× větší než kmen** — jedna prasklina byla širší než celý kmen. `kuraZrno` je proto nově **rozestup prasklin v pixelech** (dlaždice má 8 buněk na 256 px → měřítko = zrno/32). Po opravě: **35 529 pixelů se změní, průměrně o 118 z 765**.
  - **Posuvníky:** `kuraFoto` 0–1 (0,75) · `kuraZrno` 2–24 px (6) · `kuraHloubka` 0–1,6 (1,0).
