...
const RAMEC = q => 'Láttu hana eiga rót í spurningu rúnarinnar: ' + (q.indexOf(':') !== -1 ? q.slice(q.indexOf(':') + 1).trim() : q);
function postav(runa, ai, si, sOtazkou) {
  vm.runInContext('__s=' + (runa.length * 7919 + ai * 31 + si) + ';', S);
  const rune = R.find(r => r.n === runa);
  let p = S.buildReadingPrompt({ name: 'Kuky', area: A.is[ai], seeking: SK.is[si], question: '', intention: '' }, rune, 'is', []);
  if (sOtazkou) {
    const q = UI.is.coll_rune[runa][3];
    const m = p.match(/Endaðu á einni [^\n]*/);
...
(async () => {
  const sys = S.buildSysPrompt(null, 'is');
  if (process.argv[2] === 'ukaz') { console.log(postav('Algiz', 1, 0, true)); return; }
  const out = [];
  for (const [runa, ai, si] of CASE) for (const arm of [0, 1]) {
    ...
    out.push({ runa, arm, area: A.is[ai], seek: SK.is[si], otazka: UI.is.coll_rune[runa][3], reading, usage: r.usage });
    console.log(runa, arm ? 'S OTÁZKOU' : 'BEZ', '|', reading);
  }
  fs.writeFileSync(__dirname + '/otazka_runy_pilot_is.json', JSON.stringify(out, null, 1));
})();
