...
const RAMEC = q => 'Let that last line grow out of the question this rune holds: ' + (q.indexOf(':') !== -1 ? q.slice(q.indexOf(':') + 1).trim() : q);
...
(async () => {
  ...
  fs.writeFileSync(__dirname + '/otazka_runy_pilot.json', JSON.stringify(out, null, 1));
})();
