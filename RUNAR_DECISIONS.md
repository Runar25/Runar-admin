...
## 2026-09-22 (7) — Z popisů run do promptu: Prázdná runa, Perth „ještě", Mannaz zrcadlo v člověku (v4.43)
**Rozhodl:** KUKY („udělej" ke třem věcem vytaženým z `RUNAR_POPISY_RUN.md`) · **Provedl:** CODE-tune
**Co:**
- **Prázdná runa** dostává v single **vlastní esenční rám** (`ESSENCE_BLANK` / `_IS` v `runar-utils.js`) místo losu. Oba dosavadní rámy říkají, co runa *dělá* — [1] přímo „runa je v té scéně ta, která něco dělá: hýbe, drží, otevírá, nese". U Blank to vyrábělo přesně to, co popis zakazuje (*„Nehledej význam tam, kde zatím žádný není"*): čtení 2026-09-22 15:13 → „Blank holds the line still there". Nový rám runu jednou pojmenuje jako kámen bez znaku a nechá prázdné místo otevřené. **Odebrána vada, ne přidán hlídač** (memory `oprava-promptu-odebira-vadu`). V `_promptDraws` se zapisuje `essence = 'blank'`.
- **Perth** — nový obraz *„Lokið er enn á pottinum, en ilmurinn segir að eitthvað sé að verða til."* / *„The lid is still on the pot, but the smell says something is taking shape."* (aspekt *hið hulda / the unseen*). Popis: *„ne nic nevím, ale ještě nevím"* — hranice zrcadlo/orákulum. Zbylé tři obrazy Perth nesou „nevíš" (kamínek zmizí, hladina se zavře, stopy zmizí); dnešní čtení 12:54 skončilo „a thing already decided".
- **Mannaz** — nový obraz *„Andlit þitt speglast í augum hins, lítið og skýrt."* / *„In the eyes of the other your face is mirrored, small and clear."* (aspekt *mannleg vitund / humanity*, motiv `reflection`). KUKY: *„obraz o zrcadle by byl zajímavý"*; popis stojí na zrcadle **v druhém člověku**, banka měla zrcadlo jen jako vodní hladinu.
**Hranice (§13 — cesta zvážena):** rám Blank jen v **single**. Spready esenční řádek nemají od v4.9 a jména run v textu neříkají, takže tam rozpor „runa koná" nevzniká. Životní runa Blank nikdy není (`calcLifeRune` bere `RUNES[0..23]`).
...
## 2026-09-24 (7) — Rozbor GPT-6 sol: rubrika v1, jazyk podle admina, délku počítá appka · Kolekce: nadpis a písmo
**Rozhodl:** KUKY 2026-09-24 (k prvnímu živému rozboru: „1 — o ničem, počítat slova můžeš ty · 2 ok · 3 — tady ustřeluje sám · 4–5 k ničemu · musíme mu lépe stanovit, co má hledat"; „anglicky pro Sigrún (info@agndofa.is)"; Kolekce: „jiný font, mělo by být odděleno výrazněji — how Rúnar sees the rune") · **Provedl:** CODE-tune
**Co:** rubrika hledá jen šest druhů chyb (tvrzení o člověku jako fakt · chybějící runa · obraz v rozporu se zadáním — detaily vyrůstající z obrazu NEJSOU chyba · opis ze zadání · rada/poučování/klišé · Ask neodpověděl), každou s citací; nesoudí délku, gramatiku, nenavrhuje přepisy, bez markdownu. Délku čtení proti rozpočtu zadání počítá appka a ukáže ji nahoře. Jazyk rozboru podle přihlášeného admina (`GPT_REVIEW_LANG`: info@ → angličtina, jinak čeština). Kolekce: text runy pod nadpisem „✦ HOW RÚNAR SEES THE RUNE" / „✦ HVERNIG RÚNAR SÉR RÚNINA", stejným písmem jako Rúnarovo učení.
**Ověřeno:** jeden rozbor gpt-6-sol na ownerově čtení Isy (se skutečným obrazem čtení): 6 bodů s citacemi, mj. „In your home the talk has gone flat and careful" jako tvrzení o domácnosti a dům/trubky, které nejsou v obrazu. Cena ~0,2 centu za rozbor (1647 vstupních + 469 výstupních tokenů). Délka a přepnutí jazyka ověřeny bez API.
**Affected doc(s):** `RUNAR_BACKLOG.md` (schválené znění statického čtení Dagaz — na později) — v tomtéž commitu.

## 2026-09-24 (8) — Rozbor čtení levněji: gpt-6-luna místo gpt-6-sol, bez přemýšlení, bez Rúnarova systémového promptu
**Rozhodl:** KUKY 2026-09-24 („0,2 centu je hodně moc… musí to být levnější nebo použít levnější model") · **Provedl:** CODE-tune
**Oprava čísla:** dřívější „0,2 centu" byl chybný odhad; podle ceníku v měřicích skriptech (`docs/eval/2026-09-22-modely/skripty/run.js`: sol 2 $/10 $, luna 0,10 $/0,50 $ za milion) stál rozbor se solem a přemýšlením ~0,8 centu.
