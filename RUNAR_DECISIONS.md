...
- **Addendum 2026-07-14 (owner feedback) — jméno vynechat ≥50 %:** pool měl omit jen 1 ze 4 slotů → jméno padalo v **75 %** čtení. KUKY: „jméno v každém čtení otravuje". `_namePlacement` nyní vynechá jméno **~55 %** (práh `Math.random() < 0.55`, laditelný klidně výš), zbytek rovnoměrně early/middle/late. Ověřeno probe × 20 000: **EN omit 54.9 %, IS 54.7 %**, zbylé 3 varianty ~15 % každá. ⚠️ Vazba je **pozicová** (`pool[pool.length-1]`) → invariant zapsán v komentáři: varianta „vůbec" musí zůstat **POSLEDNÍ** v obou poolech, jinak se poměr tiše rozbije. Součást vlny v0.9.
- **Addendum 2026-07-14 — Ask UI copy:** `ask_placeholders[0]` („Ask Rúnar about this reading…" / „Spyrðu Rúnar um þennan lestur…") jen **duplikoval label `ask_lbl`** nad inputem → vyhozen (EN+IS), zbyly 3 reálné otázky. Label `ask_lbl` ponechán (dává kontext, co box dělá) — **čeká na copy rozhodnutí ownera**, jestli ho zjednodušit/zrušit. Jméno v Ask UI není (ověřeno: `ask_lbl`/placeholdery ho neobsahují, `buildAskPrompt` ho nebere) → do Ask se dostane jen skrz text čtení, který B9 refine ztenčil.
